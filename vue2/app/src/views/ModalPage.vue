<template>
  <div>
    <Button
      ...
      :key="'button' + index"
      @click="() => addModal(modal)"
      size="large"
    >
      {{ modal.name }}
    </Button>
    <div
      v-for="(modal, index) in getModals"
      :key="'modal' + index"
      style="margin: 10px"
    >
...
export default {
  ...
  mounted() {
    ...
    this.$root.$on("removeModal", () => this.removeModal());
    this.fetchAllModalsAction();
  },
  methods: {
    ...mapActions("modals", [
      "addModalAction",
      "removeModalAction",
      "fetchAllModalsAction",
    ]),
    addModal(modal) {
      this.addModalAction(modal);
    },
    removeModal() {
      this.removeModalAction();
    },
  },
};
</script>
